...
function seed(){
  manualPush(state.users, { 
    id: nextId('U','user'), 
    fullName: 'System Administrator', 
    username: 'admin', 
    password: 'admin123', 
    recoveryCode: ADMIN_RECOVERY_CODE,
    role: 'Admin',
    employeeId: 'N/A'
  });
}
...
function deleteUser(userId){
  ...
  if(state.users[idx].username === 'admin') return { ok:false, message:'Default admin account cannot be deleted.' };
  ...
  tbody.innerHTML = state.users.length ? state.users.map(u => `
    <tr>
      ...
      <td>
        ${u.username === 'admin' 
          ? 'Protected' 
          : `<button type="button" class="btn-logout" style="width:auto; padding:4px 8px;" onclick="uiDeleteUserDirect('${u.id}')">Delete</button>`
        }
      </td>
    </tr>
...
function renderTransactions(){
  ...
  tbody.innerHTML = state.transactions.length ? state.transactions.slice().reverse().map(t => `
    <tr>
      ...
      <td>
        ${t.status === 'Unpaid' ? `<button type="button" class="btn-primary" style="width:auto; padding:4px 8px; background:#10b981;" onclick="markTransactionAsPaid('${t.id}')">Mark Paid</button>` : '—'}
      </td>
    </tr>
...
function renderDelivery(){
  ...
  tbody.innerHTML = state.deliveries.length ? state.deliveries.map(d => `
    <tr>
      ...
      <td>
        <button type="button" class="btn-primary" style="width:auto; padding:4px 8px; background:#10b981;" onclick="setPaymentStatus('${d.id}', 'Paid')">Paid</button>
        <button type="button" class="btn-primary" style="width:auto; padding:4px 8px; background:#f59e0b;" onclick="setPaymentStatus('${d.id}', 'Unpaid')">Unpaid</button>
      </td>
      <td>
        <button type="button" class="btn-primary" style="width:auto; padding:4px 8px;" ${(d.status !== 'Paid' && d.status !== 'Unpaid') ? 'disabled style="opacity:0.5;"' : ''} onclick="markDelivered('${d.id}')">Mark Delivered</button>
      </td>
    </tr>
...
function renderInventory(){
  ...
  tbody.innerHTML = state.inventory.length ? state.inventory.map(i => {
    ...
    return `<tr>
      ...
      <td>${s.label}</td>
      <td>
        <button type="button" class="btn-primary" style="width:auto; padding:4px 8px;" onclick="editInventoryItem('${i.id}')">Modify</button>
        <button type="button" class="btn-logout" style="width:auto; padding:4px 8px;" onclick="uiRemoveInventoryItem('${i.id}')">Remove</button>
      </td>
    </tr>`;
...
function renderReceiving(){
  ...
  tbody.innerHTML = state.receiving.length ? state.receiving.slice().reverse().map(r => `
    <tr>
      ...
      <td>${r.status}</td>
      <td>${r.status==='Verified' ? '' : `<button type="button" class="btn-primary" style="width:auto; padding:4px 8px;" onclick="verifyReceiving('${r.id}')">Verify</button>`}</td>
    </tr>
...
function renderEmployees(){
  ...
  tbody.innerHTML = rows.length ? rows.map(e => `
    <tr>
      ...
      <td>
        <button type="button" class="btn-primary" style="width:auto; padding:4px 8px;" onclick="editEmployee('${e.id}')">Modify</button>
        <button type="button" class="btn-logout" style="width:auto; padding:4px 8px;" onclick="deleteEmployee('${e.id}')">Delete</button>
      </td>
    </tr>
...
function renderSuppliers(){
  ...
  tbody.innerHTML = rows.length ? rows.map(s => `
    <tr>
      ...
      <td>
        <button type="button" class="btn-primary" style="width:auto; padding:4px 8px;" onclick="editSupplier('${s.id}')">Modify</button>
        <button type="button" class="btn-logout" style="width:auto; padding:4px 8px;" onclick="deleteSupplier('${s.id}')">Delete</button>
      </td>
    </tr>
...
function buildNav(role){
  ...
  for(let i=0; i<NAV_ITEMS.length; i++){
    ...
    if(!item.adminOnly || role === 'Admin'){
      ...
      html += `<li><button type="button" id="nav-btn-${item.id}" data-section="${item.id}" onclick="showSection('${item.id}')">${item.label}</button></li>`;
    }
  }
  nav.innerHTML = html;
}
